...
function Video(): React.JSX.Element {
  // 文案生成状态
  const [inputVideoUrl, setInputVideoUrl] = useState('') // 用户输入的视频URL
  const [videoUrl, setVideoUrl] = useState('') // 下载后的视频预览URL
  const [_videoFilePath, setVideoFilePath] = useState('') // 下载后的视频文件路径
  const [isUploadMode, setIsUploadMode] = useState(false) // 是否为上传模式
  const [_selectedVideoFile, setSelectedVideoFile] = useState<File | null>(null) // 选择的本地视频文件
  ...
  const [showTranslatedInTextarea, _setShowTranslatedInTextarea] = useState(false) // 是否在新文案文本域显示译文
  ...
  const [_referenceAudioPromptText, _setReferenceAudioPromptText] = useState('') // 当前选择音色的提示文本
  const [audioSpeed, setAudioSpeed] = useState(1.0) // 语速，新接口参数，默认 1.0
  const [audioDelaySeconds, setAudioDelaySeconds] = useState(1) // 音频开头静音延迟时长（秒），实现延迟播放效果
  // 音色列表
  ...
  // 当前正在处理的功能类型（用于控制哪个按钮显示loading）
  const [activeProcessingType, _setActiveProcessingType] = useState<'title' | 'subtitle' | 'bgm' | null>(null)
  const [processingProgress, _setProcessingProgress] = useState(0) // 统一的处理进度

  // BGM状态
  const [bgmVolume, setBgmVolume] = useState(0.5)
  const [selectedBgm, setSelectedBgm] = useState('') // 选中的BGM ID
  const [_isAddingBgm, _setIsAddingBgm] = useState(false) // 是否正在添加BGM
  const [_bgmProgress, _setBgmProgress] = useState(0) // BGM添加进度
